...
import com.seraphia.seraphia_ecommerce.repository.SizeRepository;
...
// el objetivo es Que nosotros no digamos como se construye el objeto sino que solo consumamos dicho objeto
    ...
    @Override
    public Size getSizeById(Long id_size) {
        return sizeRepository.findById(id_size).orElseThrow(
                () -> new IllegalArgumentException("La talla con el id " + id_size + "no existe")
        );//.orElseThrow -> es cuando pase una excepcion, se escribe una expresion Lambda
    }

    @Override
    public Size deleteSizeById(Long id_size) {
        //1. Devolver un objeto, para esto creamos una variable temporal para guardar el usuario eliminado;
        Size temporalSize = null;
        ...
        if(!sizeRepository.existsById(id_size)) return temporalSize;

        //Caso de exito
        temporalSize = sizeRepository.findById(id_size).get();//Con esto traemos el objeto y lo guardamos en temporalSize
        sizeRepository.deleteById(id_size); //Eliminamos el producto por el id recibido
    ...
    @Override
    public Size updateSizeById(Long id_size, Size sizeUpdated) {
        //1. Evaluar que exista el producto de dicho id "id_size" que va a ser actualizado
        //1.1 Con la clase Optional podemos editar una excepcion de null pointer exception. Tambien existe Option
        //1.1 Creamos un objeto Optional de tipo Size
        Optional<Size> optionalSize = sizeRepository.findById(id_size);
        //2.Evaluando si esta vacio para el Early Return, la palabra para lanzar excepciones dentro de un metodo es Throw
        if(optionalSize.isEmpty()) throw new IllegalArgumentException("La talla con el id " + id_size + "no existe");
        //3. Obtener el objeto original
        Size originalSize = optionalSize.get();

        //4. Evaluar el usuario que va hacer cambios, vamos a evaluar sus propiedades.
        if(sizeUpdated.getSizeName() != null) originalSize.setSizeName(sizeUpdated.getSizeName());

        return sizeRepository.save(originalSize);
    }
}
