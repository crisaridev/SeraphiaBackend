package com.seraphia.seraphia_ecommerce.controller;

import com.seraphia.seraphia_ecommerce.model.Status;
import com.seraphia.seraphia_ecommerce.model.User;
import com.seraphia.seraphia_ecommerce.service.StatusService;
import com.seraphia.seraphia_ecommerce.service.UserService;
import jakarta.validation.Valid;
import lombok.AllArgsConstructor;
import org.springframework.web.bind.annotation.*;

import java.util.List;

//Como es rest el controler es REST
@RestController
//La etiqueta para el mapeo, con el path de la url al endpoint donde apuntamos al hacer la peticion
@RequestMapping(path = "/api/status")// http://localhost:8080/api/status
//Con Lombok podremos evitar la creacion del constructor con una etiqueta @RequiredArgsConstructor
@AllArgsConstructor
public class StatusController {
    //En el mapeo de la API tenemos que traernos los metodos que tienen la logica de nuestro Servicio
    //Para la inyeccion de dependencias implementaremos userService y no la implementacion, porque la interfaz nos indica que metodos existen
    //Si los metodos cambian su logica en el implementation, los metodos siguen siendo los mismos en la interfaz userService.
    //@Qualifiers si hay mas de una implementacion

    //inyeccion de dependencias
    private StatusService statusService;

    //Peticion Get para obtener todos los estados
    @GetMapping //url a la que se hace la peticion http://localhost:8080/api/status
    public List<Status> getAllStatus(){
        return this.statusService.getAllStatus();
    }

    //Peticion Get para obtener un estado por id
    //En esta ocasion va a recibir un parametro entre llaves, va ir un placeholder para el valor del id
    @GetMapping(path = "{statusId}") //Vamos a mandar el id por la url //http:localhost:8080/api/status/id -> // http://localhost:8080/status/user/2
    //Pathvariable que de la url vamos a obtener el statusId y lo pasamos como parametro a nuestro metodo
    public Status getStatusById(@PathVariable("statusId") Long id_status){
        return this.statusService.getStatusById(id_status);
    }

    //Peticion Post
    @PostMapping//@RequestBody indica que la peticion tiene un body con informacion
    public List<Status> addStatus(@Valid @RequestBody List<Status> statuses){
        return this.statusService.addStatus(statuses);
    }

    //Peticion Delete
    @DeleteMapping(path = "{statusId}")
    public Status deleteStatusById(@PathVariable("statusId")Long id_status){
        return this.statusService.deleteStatusById(id_status);
    }

    //Peticion Put
    @PutMapping(path = "{statusId}")
    public Status updateStatusById(@Valid @PathVariable("statusId") Long id_status, @RequestBody Status statusUpdated){
        return this.statusService.updateStatusById(id_status,statusUpdated);
    }

}
